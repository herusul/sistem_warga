...
$sql_check = "SELECT m.*, c.rumah_nomor, r.ref_nama, IF(m.is_aktif=1, 'Aktif', 'Tidak Aktif') AS status_data FROM warga_rumah m 
    ...
    WHERE m.warga_rumah_id = ? AND m.warga_id = ?";
$stmt_check = mysqli_prepare($conn, $sql_check);
mysqli_stmt_bind_param($stmt_check, "ii", $id, $warga_id);
mysqli_stmt_execute($stmt_check);
$rumah = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt_check));
...
if (isset($_POST['hapus'])) {
    ...
    $sql_delete = "DELETE FROM warga_rumah WHERE warga_rumah_id = ?";
...
<div class="container mt-4">
    <h4>🗑️ Konfirmasi Hapus Rumah Warga</h4>
    ...
    <div class="alert alert-warning">
        <strong>⚠️ Apakah Anda yakin ingin menghapus data rumah warga berikut?</strong><br>
        <ul>
            <li><strong>Nomor Rumah:</strong> <?= e($rumah['rumah_nomor']) ?></li>
            <li><strong>Status Rumah:</strong> <?= e($rumah['ref_nama']) ?></li>
            <li><strong>Tanggal Mulai Tinggal:</strong> <?= e($rumah['warga_rumah_tanggal']) ?></li>
        </ul>
    </div>
